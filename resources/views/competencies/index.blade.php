...
<div class="text-left mt-4">
    ...
    <select name="company" id="company" class="form-control"
                ...
                @else
                <option value="{{ url('competencies/') }}">Seluruh Perusahaan</option>
                @foreach ($company as $item)
                    <option value="{{ url('competency/company', $item->id) }}"
            ...
            <a href="{{ route('competencies.create') }}" class="d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
                    class="fas fa-plus fa-sm text-white-50"></i> Tambah Kompetensi</a>
             </div>
    </div>
